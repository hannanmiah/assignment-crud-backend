...
describe('listing', function () {
    ...
    test('per_page query',function (){
        $user = User::factory()->create();
        Product::factory()->count(15)->create();

        $response = $this->actingAs($user)->getJson('/api/products?per_page=5');
    ...
    test('search query',function (){
        ...
        $response = $this->actingAs($user)->getJson('/api/products?search=test');

        $response->assertOk();
        // assert json has one data
        expect($response->json('data'))->toHaveCount(1);
    });
    test('sort query',function (){
        $user = User::factory()->create();
        Product::factory()->count(15)->create();

        $response = $this->actingAs($user)->getJson('/api/products?sort=price:desc');
